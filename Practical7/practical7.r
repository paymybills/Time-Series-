...
bank_data <- scan("/home/staff/Documents/230957114/TSA/Practical7/bank_case.txt")
...
# List all PNG plot files in the plots/ directory
cat("\n========== Generated Plot Files =========\n")
plot_files <- list.files("plots", pattern = "\\.png$", full.names = FALSE)
if (length(plot_files) > 0) {
    for (f in plot_files) cat(file.path("plots", f), "\n")
} else {
    cat("No PNG plot files found in plots/ directory.\n")
}
cat("\n========== Model Selection and Fitting ==========\n")

# (l) Model selection using ACF/PACF and AIC
...
tryCatch({
    ...
    cat("\n========== Package Installation Required ==========\n")
    cat("The 'forecast' package is not installed.\n")
    cat("To install it, run in R console:\n")
    cat("  install.packages('forecast')\n\n")
})

if (autoarima_available) {
    cat("\nUsing auto.arima to select the best ARIMA model based on AIC...\n")
    # Use `approximation=FALSE` to improve selection robustness
    best_model <- auto.arima(bank_ts, approximation = FALSE, trace = FALSE)
    cat("\nSelected Model:\n")
    print(best_model)
    cat("\nAIC:", AIC(best_model), "\n")
    cat("\nModel coefficients:\n")
    print(coef(best_model))
    # (m) Fit the data using the selected model (already done by auto.arima)
    # (n) Goodness of fit: residual diagnostics
    cat("\n========== Residual Diagnostics ==========\n")
    png("plots/plot5_residuals.png", width = 800, height = 800)
    par(mfrow = c(2,2))
    plot(residuals(best_model), main = "Residuals", col = "purple")
    ...
    cat("\nIf p-value > 0.05, residuals are uncorrelated (good fit).\n")
} else {
    cat("\n'forecast' package not available. Fitting ARIMA(1,1,1) as example.\n")
    arima_model <- arima(bank_ts, order = c(1,1,1))
    print(arima_model)
    cat("\nAIC:", arima_model$aic, "\n")
    png("plots/plot5_residuals.png", width = 800, height = 800)
    par(mfrow = c(2,2))
    plot(residuals(arima_model), main = "Residuals", col = "purple")
    ...
    dev.off()
    cat("\nLjung-Box test for residual autocorrelation:\n")
    print(Box.test(residuals(arima_model), lag = 20, type = "Ljung-Box"))
    cat("\nIf p-value > 0.05, residuals are uncorrelated (good fit).\n")
}
